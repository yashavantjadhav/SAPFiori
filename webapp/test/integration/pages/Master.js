			...
			actions : {
							...
							this.waitFor({
								...
								success : function (oList) {
									...
									if (!aItemsNotInTheList.length) {
										...
										sCurrentId = aEntityData[aEntityData.length - 1].Vbeln;
									} else {
										sCurrentId = aItemsNotInTheList[0].Vbeln;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("HeaderSet", {
										Vbeln : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Vbeln"),
						title: oBindingContext.getProperty("Auart")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						viewName : sViewName,
						id : "page",
						matchers : new PropertyStrictEquals({name : "title", value : "<HeaderSet> (0)"}),
						success : function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("Auart");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all HeaderSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "page",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "title", value : "<HeaderSet> (" + iExpectedLength + ")"}),
								success : function () {
